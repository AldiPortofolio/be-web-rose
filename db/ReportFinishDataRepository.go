package db

import (
	"fmt"
	"github.com/jinzhu/gorm"
	"rose-be-go/models/dbmodels"
	"strings"
	"time"
)

type ReportFinishDataRepository struct {

}

func InitReportFinishDataRepository() *ReportFinishDataRepository {
	return &ReportFinishDataRepository{}
}

func init() {
	transactionDateLayout = "2006-01-02T15:04:05.000000Z07:00"
}

// GetDataReportFinished ..
func (repo *ReportFinishDataRepository) GetDataReportFinished(req dbmodels.ReportFinished) ([]dbmodels.ReportFinished, int, error) {
	db := GetDbCon()

	page := req.Page
	limit := req.Limit

	var dataReportFinish []dbmodels.ReportFinished
	var total int

	repo.GetDataPage(&db, req.StartDate, req.EndDate)

	err := db.Limit(limit).Offset((page - 1) * limit).Order("id desc").Find(&dataReportFinish).Limit(-1).Offset(0).Count(&total).Error // query

	// cara lain kalo QUEry (bukan pake paetik satu, tapi di bawah esc)
	//err := db.Exec(`query nya disni aja`).Find(&dataUploadNmid).Error

	if err != nil {
		fmt.Println("<<< Error get data Report Finish >>>")
		return dataReportFinish, 0, err
	}

	fmt.Println("<<< Error get data Report Finish >>> {}", dataReportFinish)

	return dataReportFinish, total, nil
}

// GetDataPage ...
func (repo *ReportFinishDataRepository) GetDataPage(db **gorm.DB, startDate string, endDate string) {

	if strings.TrimSpace(startDate) != "" && strings.TrimSpace(endDate) != "" {
		dateStart, errStart := time.Parse("2006-01-02", startDate)
		if errStart != nil {
			fmt.Println("Failed to parse request start date to time:", errStart)
		}

		dateEnd, errEnd := time.Parse("2006-01-02", endDate)
		if errEnd != nil {
			fmt.Println("Failed to parse request start date to time:", errEnd)
		}

		*db = (*db).Where("start_date >= ? and end_date <= ?", dateStart.Format(transactionDateLayout), dateEnd.Format(transactionDateLayout))
	}
}
